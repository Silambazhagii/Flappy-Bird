...
const lose_phrases = [
    ...
    "Flap flub! Another attempt?"
];

const win_game = [
    "Congratulations! You soared past 100!",
...
if (score === "100") {
    const Display = win_game[Math.floor(Math.random() * win_game.length)];
    words.textContent = Display;
} else {
    const randomDisplay = lose_phrases[Math.floor(Math.random() * lose_phrases.length)];
